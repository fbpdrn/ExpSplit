...
\begin{itemize}
    \item \textbf{IDE:} \texttt{IntelliJ IDEA Ultimate} per lo sviluppo del sistema e \texttt{XCode} per l'applicazione mobile.
    \item \textbf{Backend:} \texttt{Spring Boot} con i moduli \texttt{Web}, \texttt{Security} e ulteriori dipendenze per l'implementazione della sicurezza e dell'autenticazione.
    \item \textbf{Build tool:} \texttt{Gradle} per la gestione delle dipendenze e del ciclo di build.
    \item \textbf{Database:} \texttt{PostgreSQL} per la persistenza dei dati.
    \item \textbf{Database per i test:} \texttt{H2 in-memory}, per eseguire test unitari e di integrazione senza dipendere dal database di produzione.
    \item \textbf{Testing unitario:} \texttt{JUnit} per la verifica dei singoli componenti.
    \item \textbf{Testing di integrazione:} \texttt{MockMvc} combinato con JUnit per testare i flussi tra controller, servizi e layer di persistenza.
    \item \textbf{Frontend:} Applicazione demo sviluppata in \texttt{Swift} e \texttt{SwiftUI}.
    \item \textbf{Strumenti di test API:} \texttt{Postman} / \texttt{curl}.
    \item \textbf{Versionamento:} \texttt{Git + GitHub} per il controllo della versione del codice sorgente.
    \item \textbf{Deployment/Container:} \texttt{Docker} per creare ambienti coerenti tra sviluppo e produzione, e \texttt{Docker Compose} per orchestrare i microservizi (gateway, auth, expsplit) insieme al database in ambiente locale.
\end{itemize}